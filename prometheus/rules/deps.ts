 ...
 *   - `prometheus deps:audit` (async CLI): reads package.json → queries OSV.dev
 *     → writes .prometheus/dep-cache.json
...
function loadDepCache(root: string): DepCache | null {
  const cachePath = join(root, '.prometheus', 'dep-cache.json');
...
const DEP_001: PrometheusRule = {
  ...
  explain: {
    ...
    goodExample: 'Run: prometheus deps:audit  then upgrade the flagged package.',
...
const DEP_010: PrometheusRule = {
  id: 'DEP_010',
  category: 'dep_cache_stale',
  severity: 'LOW',
  description: '.prometheus/dep-cache.json is older than 24 hours — CVE data may be outdated.',
  ...
  explain: {
    ...
    goodExample: 'Add "prometheus deps:audit" to your CI pipeline before prometheus scan.',
  ...
  detect(input: DetectInput): Finding[] {
    ...
    return [f('dep_cache_stale', 'LOW',
      `.prometheus/dep-cache.json was generated ${cache.generated.slice(0, 10)} and is stale.`,
      'Run: prometheus deps:audit  to refresh CVE data.',
      '.prometheus/dep-cache.json')];
  },
};

// ── Export ────────────────────────────────────────────────────────────────────

export const DEP_RULES: PrometheusRule[] = [
  DEP_001,
  DEP_002,
  DEP_003,
  DEP_004,
