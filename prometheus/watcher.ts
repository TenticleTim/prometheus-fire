...
import type { PrometheusConfig, Finding, ScanResult } from './types.js';
...
export function startWatcher(
  ...
  function runUpdate(): void {
    ...
    // Load the latest scan (may have been updated by prometheus scan)
    const scan: ScanResult | null = loadReport(root);
    if (!scan) {
      process.stdout.write(
        `${YELLOW}No .prometheus/report.json found. Run: prometheus scan${RESET}\n`,
      );
      return;
    }
  ...
  function handleChange(filename: string | null): void {
    ...
    // Skip .prometheus/ and node_modules
    if (rel.startsWith('.prometheus') || rel.includes('node_modules') || rel.startsWith('dist')) return;
  ...
  process.stdout.write(`${BOLD}Prometheus watch${RESET} — watching ${root}\n`);
  ...
  watcher.on('error', (err) => {
    process.stderr.write(`[prometheus watch] error: ${err.message}\n`);
  });
