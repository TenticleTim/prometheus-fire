 ...
 *
 * Stored at: .prometheus/autopilot/conventions.json
...
const CONVENTIONS_FILE = '.prometheus/autopilot/conventions.json';
...
export interface ConventionSnapshot {
  ...
  // Which Prometheus rules have fired as gate blockers in this repo
...
export function buildConventionContext(snapshot: ConventionSnapshot): string {
  ...
  if (topRules.length > 0) {
    lines.push(`  ⚠ Prometheus rules that commonly fire in this repo (avoid triggering):`);
