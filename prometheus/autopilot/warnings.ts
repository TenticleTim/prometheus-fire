...
export function checkCostCeiling(maxCostUSD: number | undefined): CheckResult {
  ...
  return {
    ...
    isHardStop: false,
    hint: set ? undefined : 'Set autopilot.maxCostUSD in .prometheus/config.json to cap spending.',
  };
}
...
export function displayWarningScreen(
  ...
  process.stdout.write('\nWARNINGS — things Prometheus cannot prevent\n\n');
  ...
  const warnings = [
    {
      ...
      body: [
        ...
        'hard. Prometheus marks the task TIMED_OUT and stops the session.',
    ...
    },
    {
      ...
      body: [
        ...
        'Prometheus retries with backoff: 10s → 30s → 90s.',
    ...
    },
    {
      ...
      body: [
        ...
        '→ Resume with: prometheus autopilot resume [PLAN_FILE]',
    ...
    },
    {
      ...
      body: [
        'API calls fail if your internet drops. Prometheus retries 3× with backoff.',
    ...
    },
    {
      ...
      body:
        hooks.length > 0
          ? [
              ...hooks.map((h) => `  ${h}`),
              'If a hook rejects a commit, Prometheus cannot bypass it.',
    ...
    },
    {
      ...
      body: [
        ...
        '  prometheus autopilot --restore-permissions',
  ...
  process.stdout.write(`    prometheus autopilot resume [PLAN_FILE]\n\n`);
  process.stdout.write(`TO CANCEL WHILE THE SESSION RUNS\n`);
  process.stdout.write(`  touch .prometheus/autopilot/.cancel\n`);
  process.stdout.write(`  — or — prometheus autopilot cancel\n\n`);
