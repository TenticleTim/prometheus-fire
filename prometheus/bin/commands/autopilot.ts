/**
 * prometheus autopilot — autonomous plan execution with governance gates.
 *
 * Sub-commands:
...
function generateReviewFile(planPath: string, issues: ParseIssue[], plan: AutopilotPlan | null): string {
  ...
  lines.push(`  prometheus autopilot start ${basename(planPath)} --yes`);
...
function buildPRDescription(session: AutopilotSession, plan: AutopilotPlan, journalPath: string): string {
  ...
  return [
    ...
    ``,
    `---`,
    `*Generated by Prometheus Autopilot — this is a DRAFT PR and cannot be merged until the checklist is complete.*`,
  ].join('\n');
}
...
async function cmdAutopilotValidate(argv: string[]): Promise<void> {
  ...
  process.stdout.write(`\nPrometheus Autopilot — Plan Validation\n`);
  ...
  if (errors.length === 0 && warnings.length === 0) {
    ...
    process.stdout.write(`To start: prometheus autopilot start ${planPath} --yes\n`);
    return;
  }

  process.stdout.write(formatIssues(issues));
  process.stdout.write(`\nReview file written to: ${reviewPath}\n`);
  process.stdout.write(`Fix all errors, then run: prometheus autopilot start ${planPath} --yes\n`);
...
async function cmdAutopilotStart(planPath: string, argv: string[]): Promise<void> {
  ...
  if (!config.autopilot?.enabled) {
    process.stderr.write(
      ...
      `Set "autopilot": { "enabled": true } in .prometheus/config.json to enable it.\n`
    );
    process.exit(1);
  }
  ...
  if (errors.length > 0) {
    ...
    process.stderr.write(`Run: prometheus autopilot validate ${planPath}\n`);
    process.exit(1);
  }
  ...
  process.stdout.write(`\n${DIVIDER}\n`);
  process.stdout.write(`Prometheus Autopilot — Session ${sessionId}\n`);
  ...
  process.stdout.write(`To cancel: touch .prometheus/autopilot/.cancel\n`);
  ...
  process.stdout.write(`  Draft PR:        prometheus autopilot open-pr ${sessionId}\n`);
  process.stdout.write(`  Discard all:     prometheus autopilot revert ${sessionId}\n\n`);
  process.stdout.write(`${DIVIDER}\n`);
...
async function cmdAutopilotResume(planPath: string, argv: string[]): Promise<void> {
  ...
  if (!session) {
    process.stderr.write(`No active session found. Start a new session with:\n  prometheus autopilot start ${planPath} --yes\n`);
    process.exit(1);
  }

  const { plan, issues } = parsePlanFile(planPath);
  if (issues.some((i) => i.type === 'error') || !plan) {
    process.stderr.write(`Plan has errors. Run: prometheus autopilot validate ${planPath}\n`);
    process.exit(1);
  }
  ...
  process.stdout.write(`PR: prometheus autopilot open-pr ${session.id}\n`);
  clearSession(root);
}
...
function cmdAutopilotCancel(): void {
  ...
  process.stdout.write(`Check status with: prometheus autopilot status\n`);
}
...
function cmdAutopilotRevert(sessionIdArg: string | undefined): void {
  ...
  if (!sessionId) {
    ...
    process.stderr.write(`Usage: prometheus autopilot revert <session-id>\n`);
    process.exit(1);
  }
...
async function cmdAutopilotOpenPR(sessionIdArg: string | undefined): Promise<void> {
  ...
  if (!session || !sessionId) {
    process.stderr.write(`No session found. Provide session ID: prometheus autopilot open-pr <session-id>\n`);
    process.exit(1);
  }
...
async function cmdAutopilotGenerate(argv: string[]): Promise<void> {
  ...
  if (!goal) {
    process.stderr.write(
      `Usage: prometheus autopilot generate "describe what you want to build"\n` +
      `Example: prometheus autopilot generate "add Stripe checkout to the Express app"\n`,
    );
    process.exit(1);
  }
...
export async function cmdAutopilot(argv: string[]): Promise<void> {
  ...
  switch (sub) {
    ...
    default:
      process.stdout.write(`
Prometheus Autopilot — Autonomous Plan Execution

  DISABLED BY DEFAULT. Enable in .prometheus/config.json:
...
PLAN FORMAT
  prometheus autopilot validate --example
  prometheus autopilot generate "describe what you want to build"

SAFETY GUARANTEES
  • All work happens on an isolation branch (never main)
  • Nothing is pushed without explicit human action
  • One-command revert: prometheus autopilot revert [session-id]
