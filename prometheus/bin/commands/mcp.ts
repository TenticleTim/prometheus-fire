/**
 * prometheus mcp:serve   — start the MCP server (stdio JSON-RPC 2.0)
 * prometheus mcp:install — add prometheus to ~/.claude/settings.json as an MCP server
 * prometheus mcp:status  — show whether MCP server is configured in Claude settings
 *
...
const SERVER_NAME = 'prometheus-governance';

function resolvePrometheus(): string {
  // Prefer a globally installed binary, fall back to npx
  try {
    const p = execSync('which prometheus', { encoding: 'utf8' }).trim();
    if (p) return p;
  } catch { /* not found globally */ }
  return 'npx prometheus';
}
  ...
  switch (sub) {
    ...
    case 'uninstall': {
      ...
      if (SERVER_NAME in mcpServers) {
        ...
        settings.mcpServers = mcpServers;
        writeSettings(GLOBAL_SETTINGS, settings);
        process.stdout.write(`Prometheus MCP Server removed from ${GLOBAL_SETTINGS}\n`);
      } else {
        process.stdout.write(`Prometheus MCP Server is not installed in ${GLOBAL_SETTINGS}\n`);
      }
      break;
    }
    ...
    case 'status': {
      ...
      if (!installed) {
        process.stdout.write(`\n  Run 'prometheus mcp:install' to enable.\n`);
      }
      process.stdout.write('\n');
      break;
    }
    ...
    default: {
      process.stderr.write(`Usage: prometheus mcp:<serve|install|uninstall|status>\n`);
      process.exit(1);
    }
  }
}
