/**
 * prometheus deps:audit  — query OSV.dev for CVEs in all npm dependencies
 *                          and write .prometheus/dep-cache.json.
 *
 ...
 *
 * Usage:
 *   prometheus deps:audit              Scan all deps + devDeps and cache results
 *   prometheus deps:audit --prod-only  Scan only production dependencies
 *   prometheus deps:audit --json       Print results as JSON instead of table
 *   prometheus deps:audit --sbom       Also write prometheus.sbom.json (CycloneDX)
 *
...
function buildSbom(packages: DepCacheEntry[], projectName: string, projectVersion: string): string {
  ...
  return JSON.stringify({
    ...
    metadata: {
      ...
      tools: [{ vendor: 'Prometheus Governance', name: 'prometheus-governance', version: '2.0.0' }],
...
export async function cmdDeps(argv: string[]): Promise<void> {
  ...
  if (sub !== 'audit') {
    process.stderr.write('Usage: prometheus deps:audit [--prod-only] [--json] [--sbom]\n');
    process.exit(1);
  }
  ...
  const cacheDir = join(root, '.prometheus');
  ...
  if (sbom) {
    ...
    writeFileSync(join(root, 'prometheus.sbom.json'), sbomContent);
    if (!json) process.stdout.write(`SBOM written to prometheus.sbom.json (CycloneDX 1.4)\n`);
  }
  ...
  if (vulnCount === 0) {
    ...
    process.stdout.write(`\nRun 'prometheus scan' to see DEP findings in context.\n`);
  }
  process.stdout.write(`Cache written to .prometheus/dep-cache.json (TTL: 24h)\n`);
}
