 ...
 *   prometheus certificate:generate --write    write to .prometheus/certificate.json
...
export async function cmdCertificate(argv: string[]): Promise<void> {
  ...
  if (verify) {
    const certPath = join(root, '.prometheus', 'certificate.json');
    if (!existsSync(certPath)) {
      process.stderr.write('certificate:generate: no certificate found at .prometheus/certificate.json\n');
      process.exit(1);
    }
  ...
  // Read version from prometheus package.json
  let version = '1.0.0';
  try {
