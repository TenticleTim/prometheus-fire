 ...
 *
 * File: .prometheus/audit.jsonl  (one JSON line per entry)
 *
 * Chain invariant: entry.hash = sha256(prevHash + JSON.stringify(entry without hash))
 *
 * Usage from hooks:
 *   prometheus agent:audit:log Write src/auth.ts --status BLOCKED --findings SEC_001
 *
...
export function auditPath(root: string): string {
  return join(root, '.prometheus', 'audit.jsonl');
}
...
export function appendAuditEntry(
  ...
  const dir = join(root, '.prometheus');
...
export function rotateAuditLog(root: string): string | null {
  ...
  const archivePath = join(root, '.prometheus', `audit-${ts}.jsonl`);
...
export function formatAuditReport(entries: AuditEntry[], title = 'Agent Audit Trail'): string {
  if (entries.length === 0) {
    return `${title}\n\nNo audit entries found. Run 'prometheus claude:govern install' to start logging.\n`;
  }
